...
import numpy as np
from oncotools.data_elements.image import Image
...
def compute_dose_mask(dose=None, roi=None, mask=None):
    ...
    or np.any(iz < 0) or np.any(iz >= dose_resliced.data.shape[0]):
        try:
            test = dose_resliced.data[iz, iy0,
...
class DoseMask(Mask):
    ...
    def compute_dvh(self, edge_voxel_weight=1.0, bins=200):
        ...
        '''
        from oncotools.data_elements.dvh import compute_dvh as cdvh
