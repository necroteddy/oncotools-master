...
class Image(object):
    ...
    def __init__(self, dim=3):
        self.dimension = dim  # Number of dimensions (typically 2 [2D] or 3 [3D])
    ...
    def allocate(self, dtype=None):
        ...
        '''
        if dtype is None:
            self.data = np.empty(shape=self.size[::-1])
        else:
            self.data = np.empty(shape=self.size[::-1], dtype=dtype)
    ...
    def transform_physical_point_to_index(self, point):
        ...
        continuous_index, inbounds = self.transform_physical_point_to_continuous_index(
            point)
        return np.rint(continuous_index), inbounds
    ...
    def interpolate_slice(self, *args):
        ...
        if ix is not None and iy is not None:
            return self.data[lower_slices, iy[0]:(iy[-1]+1), ix[0]:(ix[-1]+1)]*(1-weight) + \
                   self.data[upper_slices, iy[0]:(iy[-1]+1), ix[0]:(ix[-1]+1)]*(weight)
        else:
    ...
    def compute_resampled_indices(self, template, dim):
        ...
        coordinates = template_indices * template.spacing[dim] + template.origin[dim]
...
def load_binary_mask(file, dimX=None, dimY=None, dimZ=None):
    ...
    if dimX is not None and dimY is not None and dimZ is not None:
        # print 'Reshaping buffer from', buffer.shape, 'to',
...
def test_interpolate_pixel():
    img = Image()
    img.size = [4, 4, 4]
    img.data = np.arange(64)
    img.data.shape = (4, 4, 4)
    print img.data[0:3, 0:3, 0:3]
    print

    p = [0, 0, 0]
    print 'x000', p, img.interpolate_pixel(p)
    p = [1, 0, 0]
    print 'x100', p, img.interpolate_pixel(p)
    p = [0, 1, 0]
    print 'x010', p, img.interpolate_pixel(p)
    p = [1, 1, 0]
    print 'x110', p, img.interpolate_pixel(p)
    p = [0, 0, 1]
    print 'x001', p, img.interpolate_pixel(p)
    p = [1, 0, 1]
    print 'x101', p, img.interpolate_pixel(p)
    p = [0, 1, 1]
    print 'x011', p, img.interpolate_pixel(p)
    p = [1, 1, 1]
    print 'x111', p, img.interpolate_pixel(p)
    print
    p = [0.5, 0.5, 0.5]
    print 'Interp 1:', p, img.interpolate_pixel(p)
    p = [1.5, 1.5, 1.5]
    print 'Interp 2:', p, img.interpolate_pixel(p)
    print
...
def test_create_square(size, spacing, radius, display=False):
    ''' Input parameters specified as (X,Y,Z) '''

    sp = parse_input(spacing)
    sz = parse_input(size)
    r = parse_input(radius)

    center = [(s - 1) / 2.0 for s in sz]
    iz, iy, ix = np.mgrid[:sz[0], :sz[1], :sz[2]]
    square = (abs((iz-center[0])*sp[0]) <= r[0]) & \
             (abs((iy-center[1])*sp[1]) <= r[1]) & \
             (abs((ix-center[2])*sp[2]) <= r[2])

    m = Mask()
    m.set_image(square, spacing=sp[::-1])
    if display:
        print m
        print
        print m.data.astype(int)
        print
    return m


def test_create_circle(size, spacing, radius, display=False):
    ''' Input parameters specified as (X,Y,Z) '''
    ...
    center = [(s - 1) / 2.0 for s in sz]
    iz, iy, ix = np.mgrid[:sz[0], :sz[1], :sz[2]]
    circle = ((iz-center[0])*sp[0]/r[0])**2 + \
             ((iy-center[1])*sp[1]/r[1])**2 + \
             ((ix-center[2])*sp[2]/r[2])**2
    m = Mask()
    m.set_image(circle <= 1, spacing=sp[::-1])
    if display:
        print m
        print
        print m.data.astype(int)
...
def test_expand_contract():
    # shape = test_create_square(15, 0.1, 0.4)
    shape = test_create_circle(15, 0.1, 0.6)
    # shape = test_create_circle(15, 1, 6)
    print 'Original Mask:'
    print shape
    print shape.data[7, :, :].astype(int)
    print
    for i in range(5):
        exp = (i - 2) / 10.0
        if exp < 0:
            print 'Contraction:', exp
            new_mask = shape.contract(-exp)
        else:
            print 'Expansion:', exp
            new_mask = shape.expand(exp)
        # print new_mask.data[7,:,:].astype(int)
        for j in range(new_mask.data.shape[0]):
            print '**', j, '**'
            print new_mask.data[j, :, :].astype(int)
        print
...
if __name__ == '__main__':
    ...
    # test_interpolate_pixel()
    # test_create_square((15,15,1), 1, (2,5,1), True)
    # test_create_circle((15,15,1), 1, (2,5,1), True)
