...
def parse_contour_data(*args):
    ...
    if len(args) == 0:
        ...
        data, plane = None, None
        print 'ERROR: Failed to initialize contour. Incorrect number of input parameters'
        print
    return data, plane
...
class Roi(object):
    ...
    def __init__(self, name=None, volume=None, contours=None, msk=None):
        ...
        self.header = {}  # ROI meta-data (data format-specific, e.g., DICOM, Pinnacle, etc.)
    ...
    def check_contour_directions(self):
        ...
        for plane, plane_contours in contours.iteritems():
            # print 'Plane:', plane
            ...
            for i, p1 in enumerate(paths):
                for j, p2 in enumerate(paths):
                    if i != j:
                        ...
                        # print 'Path '+str(i)+' contains path '+str(j)+':', contains_path[i,j]
    ...
    def get_mask(self,
        ...
        for cntr in self.contours:
            ...
            # on the mask
            lowerIdx, inbounds = mask.transform_physical_point_to_index(
                cntr.min())
            upperIdx, inbounds = mask.transform_physical_point_to_index(
                cntr.max())
            ...
            if map_points_to_voxels:
                ...
                contour_idx, inbounds = mask.transform_physical_point_to_index(
                    cntr.data)
    ...
    def get_mask_information(self, size=None, spacing=None):
        ...
        for cntr in self.contours:
            roiMin = np.min(roiMin, cntr.min())
            roiMax = np.max(roiMax, cntr.max())
        mask.origin = roiMin
        mask.end = roiMax
    ...
    def get_edge_mask(self, mask=None):
        ...
        for cntr in self.contours:
            ...
            points, inbounds = mask.transform_physical_point_to_continuous_index(
                cntr.data)
            points = points + 0.5  # With respect to the lower corner of the voxel, not the voxel center
            indexed_cntr = Contour(points)

            # Interpolate the current contour points at mask voxel edges
            final_points, is_boundary = self.interpolate_contour_at_voxel_edges(cntr, mask)
            ...
            while i < num_points:
                ...
                edge_mask.data[indexed_cntr.plane, lower_corner[1],
                               lower_corner[0]] += area
                # print 'Voxel area inside contour:', area
    ...
    def compute_edt(self, msk=None, limits=None):
        """
        ...
        while idx:
            ...
            for dim in range(ND):
                # print '*** Iteration {0}, Dim {1}'.format(counter,dim)
                for offset in [-1, 1]:
                    # print '*** Iteration {0}, Dim {1}, Offset {2}'.format(counter,dim,offset)
