...
def visualize_masks(msks, cameraElev=None, cameraAzim=None, alpha=None):
    '''
    3D Scatter plot of mask using matplotlib.
    Use this method for quickly visualizing data while debugging or performing data validation.
    Keyword arguments specify the location of the camera for the 3D plot.
    ...
    for i, msk in enumerate(msks):
        pts = msk.data.nonzero()
        ax.scatter(pts[0], pts[1], pts[2], alpha=alpha[i], color=colors[i])
        ax.view_init(elev=cameraElev, azim=cameraAzim)
    ax.set_xlabel('Z')
    ax.set_ylabel('Y')
    ax.set_zlabel('X')
...
def visualize_dose_mask(dose_map,
    ...
    ptsAndDoses = [(pts[0][ind], pts[1][ind], pts[2][ind],
                   ...
                   for ind, val in enumerate(pts[0])]
    ...
    for ind, tup in enumerate(ptsAndDoses):
        ...
        ax.scatter(tup[0], tup[1], tup[2], depthshade=False, c=arr)

    ax.view_init(elev=cameraElev, azim=cameraAzim)
    plt.xlabel(zLabel)
    plt.ylabel(yLabel)
    plt.show()
    scalarMap.set_array([5, 10])
    cbar = fig.colorbar(scalarMap, ticks=[5, 10], orientation='horizontal')
